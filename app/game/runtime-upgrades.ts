...
const INSTALL_KEY=Symbol.for("bubblehex.runtime-upgrades.v1");
...
function restoreCheckpoint(engine:AnyEngine,checkpoint:RuntimeCheckpoint){
  engine.hero=checkpoint.hero;engine.selected=checkpoint.hero;engine.score=checkpoint.score;engine.lives=3;
  engine.venom=new Set(checkpoint.venom);engine.upgrades={...checkpoint.upgrades};engine.levelIndex=checkpoint.levelIndex;
  engine.loadLevel(checkpoint.levelIndex);
  engine.message=`CHECKPOINT RESTORED · STAGE ${checkpoint.levelIndex+1} · 3 LIVES`;
  engine.messageLife=3;engine.audio?.secret?.();engine.setState("stageIntro");
}

function drawHighContrastProjectile(engine:AnyEngine,p:{x:number;y:number;vx:number;vy:number;kind:"tear"|"star"}){
  const c=engine.ctx as CanvasRenderingContext2D,angle=Math.atan2(p.vy,p.vx),reduced=!!engine.settings?.reducedMotion;
  const pulse=reduced?1:1+Math.sin(performance.now()/70)*.12;
  const neon=p.kind==="tear"?"#56E7FF":"#FFD36A",edge=p.kind==="tear"?"#087CFF":"#FF2A9D";
  const tail=22,tx=p.x-Math.cos(angle)*tail,ty=p.y-Math.sin(angle)*tail;
  c.save();c.lineCap="round";
  c.strokeStyle="rgba(0,0,0,.96)";c.lineWidth=14;c.beginPath();c.moveTo(tx,ty);c.lineTo(p.x,p.y);c.stroke();
  c.strokeStyle="#FFFFFF";c.globalAlpha=.9;c.lineWidth=8;c.beginPath();c.moveTo(tx,ty);c.lineTo(p.x,p.y);c.stroke();
  c.strokeStyle=neon;c.globalAlpha=1;c.lineWidth=4;c.shadowBlur=18;c.shadowColor=neon;c.beginPath();c.moveTo(tx,ty);c.lineTo(p.x,p.y);c.stroke();
  c.translate(p.x,p.y);c.rotate(angle+(p.kind==="star"&&!reduced?performance.now()/180:0));c.scale(pulse,pulse);
  c.fillStyle=neon;c.strokeStyle="rgba(0,0,0,.98)";c.lineWidth=5;
  if(p.kind==="tear"){
    c.beginPath();c.moveTo(12,0);c.quadraticCurveTo(-1,-10,-10,0);c.quadraticCurveTo(-1,10,12,0);c.closePath();c.fill();c.stroke();
    c.strokeStyle=edge;c.lineWidth=3;c.stroke();c.strokeStyle="#FFFFFF";c.lineWidth=1.5;c.stroke();c.fillStyle="#FFFFFF";c.beginPath();c.arc(3,-2,3,0,Math.PI*2);c.fill();
  }else{
    c.beginPath();for(let i=0;i<16;i++){const r=i%2===0?12:5,a=i*Math.PI/8;if(i===0)c.moveTo(Math.cos(a)*r,Math.sin(a)*r);else c.lineTo(Math.cos(a)*r,Math.sin(a)*r)}c.closePath();c.fill();c.stroke();
    c.strokeStyle=edge;c.lineWidth=3;c.stroke();c.strokeStyle="#FFFFFF";c.lineWidth=1.5;c.stroke();c.fillStyle="#FFFFFF";c.beginPath();c.arc(0,0,3.5,0,Math.PI*2);c.fill();
  }
...
export function installBubbleHexRuntimeUpgrades(EngineClass:EngineConstructor){
  ...
  proto.clearStage=function(this:AnyEngine,demo:boolean){
    ...
    this.messageLife=3;this.audio?.recordSting?.();
  };
  ...
  proto.beginRun=function(this:AnyEngine){checkpoints.delete(this);return originalBeginRun.call(this)};

  const originalSyncAuditData=proto.syncAuditData;
  proto.syncAuditData=function(this:AnyEngine){
    originalSyncAuditData.call(this);const checkpoint=checkpoints.get(this);
  ...
  proto.drawHud=function(this:AnyEngine){
    originalDrawHud.call(this);const checkpoint=checkpoints.get(this);
